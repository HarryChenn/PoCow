...
界面支持**中英文切换**：首页右上角与牌桌头部都有语言按钮，选择会记在本地。首次打开按浏览器语言判断（中文浏览器为中文，其余为英文）。
...
| 同花 | ×2 |
| 顺子（Q+K+Joker 视为顺子） | ×2 |
| 三条（**直接成牛**，点数和无需凑 10 的倍数） | ×3 |

同一组 3 张同时满足多个加成时相乘（如 3 张同花顺 = ×4）。

三条自身即成牛：否则 3×点数 只有点数为 10 时才是 10 的倍数，A~9 的三条永远无牛、拿不到 ×3。

**手牌加成 —— 王炸**

手里有**双王**，最终赔分 **×3**，不论两张王在底牌还是踢脚。与底牌倍率**相乘叠加**，无牛也生效：

| 情形 | 赔分 |
| --- | --- |
| 底牌同花（×2）+ 踢脚双王（牌力 7） | 7 × 2 × 3 = 42 |
| 双王 + K 作底（和 30）、踢脚 4+6（牌力 5） | 5 × 3 = 15 |
| 无牛，但手里有双王 | 1 × 3 = 3 |

王炸本身只有 2 张，不能当底牌用：要把它放进底牌，仍需第三张把点数和凑成 10 的倍数（只有 10 点牌做得到）。
...
- 底牌倍率取 5 张中「加成乘积最大的**某一个** 3 张子集」；加成只在同一子集自身同时满足时相乘，**不能跨子集拼凑**。
  ...
  - 炸弹自带三条 → 12×3 = 36 分
  - 同花顺 → (8+9)×2×2 = **68 分**
- 王炸 ×3 是手牌加成，叠在子集倍率之上（五花含双王 → 10 × 2 × 3 = 60 分）。
...
src/i18n/     中英文案与本地化
  dict.ts     文案字典（en 为键的基准，zh 由类型约束补齐）
  index.tsx   语言 Context、切换器、插值
  format.ts   结构化牌型/战报 → 当前语言的文字
```
